...
This server adds a `search_docs` tool that your agent can use to retrieve
real-time API definitions and integration patterns from the official Gemini API
documentation.

Note that the `gemini-api-dev` skill works with or without the MCP server, so we
recommend installing them both.
